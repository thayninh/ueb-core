...
| Thuộc tính | Giá trị |
| --- | --- |
| Trạng thái | PLANNED — SOURCE CONTRACT HARD GATE NOT PASSED |
| Nhánh thực hiện | `feat/phase-2-database-import` |
| Người phụ trách | Chưa phân công |
| Ngày cập nhật | 2026-07-15 |
| Phạm vi môi trường | Chỉ môi trường phát triển local |
...
Thông tin dưới đây chỉ là tham chiếu từ hồ sơ Giai đoạn 0 đang ở trạng thái `Draft`; chưa cấu thành một source contract đã `PASS`:

| Thuộc tính | Giá trị tham chiếu hiện tại | Trạng thái cần đạt |
| --- | --- | --- |
| File nguồn | `data/input/ueb_core_source_2026-07-14.xlsx` | Tên file và đúng bản file phải được chủ sở hữu dữ liệu chốt |
| SHA-256 | `f3c2b19d44257ca9c1ec0bd7ce55d772a0f016686b8d5252e12ce05dda30ec52` | Checksum tính lại phải khớp và được người có thẩm quyền phê duyệt |
| Sheet import | `csdlcore` | Phải tồn tại đúng tên và được chốt là sheet import duy nhất |
| Số dòng dữ liệu | `2497` | Phải xác định rõ không tính header và khớp kết quả inspect |
| Số cột nghiệp vụ | `20` | Phải khớp kết quả inspect |
| Thứ tự chính xác của header | Chưa được ghi nguyên văn và chốt trong hồ sơ hiện tại | Phải ghi đủ 20 header nguyên văn, đúng thứ tự, không tự trim, đổi tên hoặc sắp xếp lại |
...
- [ ] Chốt đúng file nguồn.
- [ ] SHA-256 tính lại khớp và được phê duyệt.
- [ ] Chốt sheet import.
- [ ] Chốt số dòng dữ liệu.
- [ ] Chốt số cột.
- [ ] Chốt nguyên văn đủ 20 header theo đúng thứ tự.
- [ ] Inspect report đạt và không chứa dữ liệu cá nhân trong Git.
- [ ] Source contract được đánh dấu `PASS` bởi người có trách nhiệm.

**Điều kiện chuyển checkpoint:** Tất cả mục CP2-1 phải đạt trước khi tạo migration.

### CP2-2 — Thiết kế database

- [ ] Mapping đủ 20 cột nghiệp vụ được review theo source contract.
- [ ] Thiết kế cột kỹ thuật cho `record_uid`, `version_no`, snapshot, hiệu lực, danh tính, provenance và submission được review.
- [ ] Thiết kế `import_run` và `workflow_event` skeleton được review.
- [ ] Chiến lược transaction, idempotency và rollback được mô tả.
- [ ] Thiết kế role và append-only được review.
- [ ] Không tự ý thay đổi mô hình bảng nghiệp vụ sau khi được chốt; mọi thay đổi phải qua review riêng.

### CP2-3 — Migration và bảo vệ database

- [ ] Migration chạy thành công trên PostgreSQL local sạch.
- [ ] Migration có thể tái lập từ đầu bằng quy trình đã ghi nhận.
- [ ] Trigger từ chối `UPDATE` và `DELETE` trên bảng lõi.
- [ ] Runtime role chỉ có quyền tối thiểu và không thể thay đổi schema.
- [ ] Test chứng minh quyền import, migration và runtime được tách biệt.

### CP2-4 — Inspect và dry-run

- [ ] Inspect kiểm tra đủ sáu thành phần hard gate.
- [ ] Dry-run không làm thay đổi database.
- [ ] Dry-run báo cáo đầy đủ unresolved identity và duplicate candidate mà không loại dòng.
- [ ] Mọi lỗi contract trả kết quả `FAIL` rõ ràng và dừng xử lý.

### CP2-5 — Import local

- [ ] Chỉ dùng file có source contract `PASS`.
- [ ] Import chạy trong transaction và không để lại dữ liệu lõi nhập dở khi lỗi.
- [ ] `import_run` ghi đủ provenance và số liệu kiểm soát.
- [ ] Toàn bộ dòng legacy được giữ theo chính sách đã nêu.
- [ ] Không kết nối hoặc ghi dữ liệu production.

### CP2-6 — Verification

- [ ] Đối chiếu contract, tổng số dòng, đủ 20 cột và từng dòng đều `PASS`.
- [ ] Các invariant legacy về hiệu lực, `version_no`, `source_submission_id`, snapshot và `workflow_event` đều `PASS`.
- [ ] Số lượng unresolved identity và duplicate candidate cân bằng với báo cáo inspect/dry-run.
- [ ] Append-only và quyền runtime được kiểm thử thành công.
- [ ] Chạy lại verify cho cùng `import_run` cho kết quả ổn định.
...
- [ ] Chỉ các artifact Phase 2 được phép mới thay đổi.
- [ ] Tài liệu runbook local và bằng chứng nghiệm thu không chứa dữ liệu cá nhân.
- [ ] Các điều kiện Phase 0 vẫn được ghi nhận là `OPEN` trừ khi có quy trình và người có thẩm quyền cập nhật riêng.
