...
- `base_version_no` đối với dòng hiện có, `NULL` đối với dòng mới;
- đủ 20 trường nghiệp vụ trong `row`;
- với `CREATE_NEW`, `row.stt` là `NULL` để biểu thị chưa được cấp, không phải placeholder;
...
| --- | --- | --- | --- |
| `CONFIRM_UNCHANGED` | `record_uid` đã có core row hiện hành của lecturer | Server copy đủ 20 trường từ phiên bản hiện hành; client không gửi business mutation | Giữ `record_uid`, `version_no = current + 1`, giữ nguyên 19 giá trị non-`stt`; `stt` mới do sequence cấp |
| `UPDATE_EXISTING` | `record_uid` đã có core row hiện hành của lecturer | Server lấy 6 trường chỉ đọc từ current row và merge đúng 14 trường editable đã validate | Giữ `record_uid`, `version_no = current + 1`, insert đúng canonical submitted row |
| `CREATE_NEW` | Logical row chưa tồn tại trong core | Server sinh `record_uid`, lấy 5 trường định danh còn lại từ canonical lecturer identity/current data, giữ `stt = NULL`, merge 14 trường editable | Dùng `record_uid` đã sinh, `version_no = 1`, bỏ `stt` khỏi `INSERT` để sequence cấp |
...
Client chỉ cung cấp 14 trường editable. Server lấy identity fields từ canonical current data của chính lecturer, sinh `record_uid`, suy ra `approval_unit` và lưu `stt = NULL` trong submission payload. Client không được cấp UUID hoặc giá trị kỹ thuật cho logical row mới.
