...
const CONFIRMATION = "--confirm-reset-phase4-leader-approve";

async function run(): Promise<void> {
  if (!process.argv.includes(CONFIRMATION)) {
    throw new Error(
      "Explicit Phase 4 leader approval E2E confirmation required.",
    );
  }
...
function runPlaywright(): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      ["test", "--config=playwright.phase-4-approval.config.ts"],
      {
        cwd: process.cwd(),
        env: process.env,
        stdio: "inherit",
    ...
    child.once("exit", (code) => {
      if (code === 0) resolve();
        ...
        reject(new Error("Phase 4 leader approval Playwright tests failed."));
    });
  });
}
  ...
  await run().catch((error) => {
    console.error(
      error instanceof Error
        ? error.message
        : "Phase 4 leader approval E2E failed safely.",
    );
    process.exitCode = 1;
  });
}
